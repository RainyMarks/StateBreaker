...
新版 CLI 不是一键演示按钮，而是一套可迁移的真实竞态测试步骤。当前用“老王奶茶券”
作为目标；以后更换 Workflow、Invariant 和目标地址即可测试其他已授权系统。
...
仓库已经包含第一版 HAR importer、竞态计划生成器、并发执行器、验证器和 PDF reporter；
它们仍是独立插件，可以由组员继续替换或扩展，而不修改核心数据契约。
